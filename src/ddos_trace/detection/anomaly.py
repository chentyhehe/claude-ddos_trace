...
class TrafficBaseline:
    ...
    2. 告警聚合阈值（alert_thresholds）:
       - 来自告警表或配置文件的硬阈值
       - 用于计算"攻击贡献度"：该 IP 的流量占告警阈值的比例
       - 一个 IP 即使未达 P95 的 1.5 倍，但贡献了告警阈值 30%+ 的流量，
         也应被标记为可疑
    ...
    # 理由: P95 已经覆盖了 95% 的正常波动，再乘 1.5 给剩余 5% 留空间
    ...
    def _compute_effective_thresholds(self, baseline_stats: Dict) -> Dict[str, float]:
        ...
        if self.traceback_config.use_dynamic_baseline:
            ...
            # 不再依赖 fallback 的告警阈值，避免默认值 500K 污染自适应计算
        ...
        # 告警聚合阈值（用于计算攻击贡献度）
        # 这是从告警表获取的阈值，代表"触发告警的总流量水平"
        # 在异常检测中用于计算每个 IP 对总攻击的贡献比例
...
class AnomalyDetector:
    ...
    def _score_contribution(
        ...
        衡量该 IP 的流量占实际攻击总量的比例。

        分母选择策略（优先级从高到低）:
        1. 告警聚合阈值（alert_pps_threshold）：如果告警表提供了真实阈值，
           代表触发告警的实际流量水平，是最准确的分母
        2. 全量数据的 PPS/BPS 总和：如果告警阈值是 fallback 默认值，
           用全量总和更准确地反映"该 IP 占实际攻击的比例"

        为什么需要这个因子:
        在分散型 DDoS 中，每个僵尸节点的 PPS 可能只有几千（相对同侪偏离不大），
        但如果几千个节点中只有一个 IP 就贡献了总流量的 5%+，它仍然是核心攻击源。
        ...
        # 判断告警阈值是否来自 fallback（默认值）
        # 如果告警阈值等于默认值，说明告警表中没有提供真实阈值
        is_pps_fallback = (
            alert_pps >= self.threshold_config.pps_threshold
            or alert_pps <= 0
        )
        is_bps_fallback = (
            alert_bps >= self.threshold_config.bps_threshold
            or alert_bps <= 0
        )

        # 分母选择:
        # - 告警阈值有效时（非 fallback）: 用告警阈值（更准确）
        # - 告警阈值是 fallback 默认值时: 用全量数据的实际总和（更合理）
        total_pps = pps.sum()
        total_bps = bps.sum()

        pps_denom = alert_pps if not is_pps_fallback else max(total_pps, 1)
        bps_denom = alert_bps if not is_bps_fallback else max(total_bps, 1)

        # 取 PPS 和 BPS 中贡献度更大的维度
        pps_contrib = pps / pps_denom
        bps_contrib = bps / bps_denom
        ...
        # 分段评分: 贡献度越大分数越高
        score = np.where(
            contribution >= 0.20,                                           # ≥20%: 核心攻击源
            np.clip(70 + (contribution - 0.20) / 0.80 * 30, 70, 100),
            np.where(
                contribution >= 0.05,                                       # 5%-20%: 显著贡献
                40 + (contribution - 0.05) / 0.15 * 30,
                np.where(
                    contribution >= 0.01,                                   # 1%-5%: 一定贡献
