...
class ThreatIntelDashboardRepository:
    ...
    def get_dashboard(self, recent_limit: int = 12) -> Dict[str, object]:
        ...
        overview_rows = self._select_clickhouse(
            f"""
            SELECT
        ...
        daily_trend = self._select_clickhouse(
            f"""
            SELECT
        ...
        attack_type_distribution = self._select_clickhouse(
            f"""
        ...
        target_hotspots = self._select_clickhouse(
            f"""
            SELECT
                target_ip,
        ...
        monitor_hotspots = self._select_clickhouse(
            f"""
            SELECT
        ...
        source_class_distribution = self._select_clickhouse(
            f"""
        ...
        top_isps = self._select_clickhouse(
            f"""
            SELECT
    ...
    def get_event_detail(self, event_id: str) -> Optional[Dict[str, object]]:
        ...
        artifact_gallery = self._build_artifact_gallery(str(event.get("output_dir", "") or ""))
        ...
        return {
            ...
            "artifacts": artifact_gallery["all"],
            "artifact_images": artifact_gallery["images"],
            "artifact_reports": artifact_gallery["reports"],
            "artifact_tables": artifact_gallery["tables"],
        }
    ...
    def list_events_filtered(
        ...
        # 攻击类型
        if attack_type:
            where_clauses.append("has(attack_types, %(attack_type)s)")
        ...
        sort_map = {
            "time": "start_time",
            "severity": "lengthInBytes(severity)",
            "bps": "peak_bps",
    ...
    def get_top_repeat_sources(self, limit: int = 50, min_events: int = 2) -> Dict[str, object]:
        ...
        # 补充情报命中
        enriched = self._build_source_intel(items)
    ...
    def get_blacklist_assets(self, status: str = "active", page: int = 1, page_size: int = 20) -> Dict[str, object]:
        try:
            ...
            items = self._select_mysql(
                "SELECT * FROM ti_blacklist WHERE indicator_type = 'ip' AND status = %s ORDER BY created_time DESC LIMIT %s OFFSET %s",
